	...
	def __init__(self, PlatformName, Arch=None, wwiseSdkRootDir=None):
		...
		if sys.platform == 'darwin':
			if "akm" in socket.gethostname().lower():
				...
				self.Paths['SWIG_CSharpModule'] = str((Path.home() / 'homebrew' / 'opt' / 'swig' / 'share' / 'swig' / '4.1.1' / 'csharp').resolve())
			else:
				self.Paths['SWIGEXE'] = '/usr/local/bin/swig'
				self.Paths['SWIG_CSharpModule'] = '/usr/local/Cellar/swig/4.1.1/share/swig/4.1.1/csharp'
			...
			self.Paths['SWIGEXE'] = '/usr/local/bin/swig'
			self.Paths['SWIG_CSharpModule'] = '/usr/local/share/swig/4.1.1/csharp'
			self.Paths['SWIG_WcharModule'] = self.NormJoin(self.Paths['SWIG_CSharpModule'], 'wchar.i')
			self.Paths['Premake_exe'] = self.NormJoin(self.Paths['Wwise_Root'], 'Tools', 'Linux', 'bin', 'premake5')
		else:
			self.Paths['SWIG'] = 'c:\\swigwin-4.1.1'
...
def Init(argv=None):
	...
	parser.add_argument('-a', '--arch', nargs=1, dest='arch', default=None, help='The target architecture to build for certain platforms, available options: Windows: {}, Android: {}; default to None if none given.'.format(', '.join(GetSupportedArches('Windows')), ', '.join(GetSupportedArches('Android'))))
	Undefined = 'Undefined'
